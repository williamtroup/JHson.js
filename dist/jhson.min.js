...
function O(a,b){if(w(a)&&u(b.json)){var c=M(b.json),e={css:{},templateDataKeys:[],templateDataKeysLength:0,templateDataKeysProcessed:[]};if(b.clearCssFromHead)for(var q=[].slice.call(x.getElementsByTagName("styles")),l=q.length,f=0;f<l;f++)q[f].parentNode.removeChild(q[f]);w(b.templateData)&&X(b,e);if(c.parsed&&w(c.result)){for(var m in c.result)if(m===a.nodeName.toLowerCase()){if(b.removeOriginalAttributes)for(;0<a.attributes.length;)a.removeAttribute(a.attributes[0].name);b.clearOriginalHTML&&(a.innerHTML=
